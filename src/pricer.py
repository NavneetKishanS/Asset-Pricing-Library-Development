...
class Pricer(ABC):
    ...
    self.finite_diff_method = FiniteDifferenceMethod.CENTRAL  # default value : HW3
    self.bump_type = BumpType.RELATIVE  # default value : HW3
    ...
    # HW3 : Method to calculate Delta and Gamma using finite differences
    def calculate_numerical_greeks(self, option, spot, rate, volatility, bump_size=0.01):
        if self.bump_type == BumpType.ABSOLUTE:
            bump_up = bump_size
            bump_down = -bump_size
        else:  # BumpType.RELATIVE
            bump_up = spot * bump_size
            bump_down = -spot * bump_size

        price_original = self.price(option, spot, rate, volatility)

        # Forward Difference
        if self.finite_diff_method == FiniteDifferenceMethod.FORWARD:
            price_up = self.price(option, spot + bump_up, rate, volatility)
            delta = (price_up - price_original) / bump_up
            gamma = None  # Gamma requires a central or backward difference

        # Backward Difference
        elif self.finite_diff_method == FiniteDifferenceMethod.BACKWARD:
            price_down = self.price(option, spot + bump_down, rate, volatility)
            delta = (price_original - price_down) / bump_down
            gamma = None  # Gamma requires a central or forward difference

        # Central Difference
        else:  # FiniteDifferenceMethod.CENTRAL
            price_up = self.price(option, spot + bump_up, rate, volatility)
            price_down = self.price(option, spot + bump_down, rate, volatility)
            delta = (price_up - price_down) / (2 * bump_size)
            gamma = (price_up - 2 * price_original + price_down) / (bump_size ** 2)
