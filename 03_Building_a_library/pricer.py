...
class EuropeanDigitalAnalyticPricer:
    def __init__(self, contract, spot, rate, volatility):
    ...
    def d1(self):
        return (np.log(self.spot / self.contract.strike) + 
                (self.rate + 0.5 * self.volatility ** 2) * self.contract.expiry) / \
                (self.volatility * np.sqrt(self.contract.expiry))

    def price(self):
        d1 = self.d1()
        if self.contract.is_call:
            return np.exp(-self.rate * self.contract.expiry) * norm.cdf(d1)
        else:
            return np.exp(-self.rate * self.contract.expiry) * norm.cdf(-d1)

    # Add methods for Greeks here if needed
